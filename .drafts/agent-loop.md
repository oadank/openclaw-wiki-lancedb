...
title: "Agent loop"
category: concepts
tags: [agent-loop, openclaw, session-management, tool-execution, queuing]
  - concepts
sources:
  - "/opt/openclaw/data/workspace/refs/openclaw-docs/docs/concepts/agent-loop.md"
summary: "OpenClaw"
read_when:
  - You need an exact walkthrough of the agent loop or lifecycle events
---

> **TL;DR** OpenClaw的代理循环提供完整的执行管道，包含消息处理、上下文组装、模型推理、工具执行和流式回复，确保会话状态一致性。

[[session management]] [[hooks]] [[command queue]] [[system prompt]]
...
1. What are the specific performance characteristics and latency implications of the per-session serialization approach?
2. How does the global lane queuing mechanism interact with session-level concurrency controls?
3. What happens when session write locks conflict with external processes accessing the same files?
4. Are there any documented failure recovery mechanisms for interrupted agent loops?
...
[^1]: /concepts/queue — Command Queue documentation for messaging channel queue modes
[^2]: /concepts/system-prompt — System prompt structure and content that models see during execution
